...
위에서는 classic 과 node 그리고 타입스크립트에서 어떻게 모듈을 찾는지에 대해서 알아보았다. 다음에 알아볼 내용은 module resolution 과 관한 설정이다. 몇몇의 flag 값으로 module resolution 방식을 변경(?)할 수 있다.

### module resolution flag

프로젝트 코드의 실행 결과는 종종 원래의 소스 코드와 완벽하게 일치하지 않는다. 일반적으로 build 과정은 결과를 도출해내고 이 빌드 과정에서 typeScript 는 javaScript 는 컴파일 된다. 그리고 여러 코드에 존재하는 의존성들은 하나의 위치에 복사되어 묶인다. 최종적으로 실행 시점에서 모듈은 소스파일이 가지고 있는 이름과 다를 수 있다. 또는 컴파일 시점에서 최종 결과의 모듈 경로는 원래의 소스 파일에 있는 모듈 경로와 정확히 일치하지 않을 수 있다.
